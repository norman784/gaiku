...
impl Node {
  ...
  fn get_mut_leaf(&mut self, point: &Vector3<f32>) -> Option<&mut Chunk> {
    if !self.boundary.contains(&point) {
      return None;
    }
    ...
    match &mut self.nodes {
      Some(nodes) => {
        for node in nodes {
          if let Some(chunk) = node.get_mut_leaf(point) {
            return Some(chunk);
          }
        }
      }
...
fn update_neighbors(node: &mut Node, leaf: &Chunk) {
  ...
  for coord in coords.iter() {
    if let Some(chunk) = node.get_mut_leaf(coord) {
