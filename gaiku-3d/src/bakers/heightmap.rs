    ...
    fn bake(chunk: &Chunk) -> Option<Mesh> {
        ...
        for x in 0..chunk.width() - 1 {
            ...
            for y in 0..chunk.height() - 1 {
                if chunk.is_air(x, y, 0) { continue; }
                let fz = y as f32;
                ...
                let left_back = Self::index(
                    &mut vertices_cache,[fx - 0.5, lb, fz - 0.5].into()
                );
                let right_back = Self::index(
                    &mut vertices_cache,[fx + 0.5, rb, fz - 0.5].into()
                );
                let right_front= Self::index(
                    &mut vertices_cache,[fx + 0.5, rf, fz + 0.5].into()
                );
                let left_front = Self::index(
                    &mut vertices_cache,[fx - 0.5, lf, fz + 0.5].into()
                );
